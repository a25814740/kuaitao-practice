...
- [x] 定義需要共享的使用者欄位（暱稱、頭像、職業、下班時間、收入）。
- [x] 建立 `user` store（`src/stores/user.ts`）。
- [x] 設定 store 預設值，確保首次載入有資料可顯示。
- [x] 建立設定 UI（`src/components/SettingPanel.vue`）。
- [x] 設定 UI 寫入 store（`v-model` 綁定 store 欄位）。
- [x] 首頁讀取 store 顯示個人化內容（下班時間倒數跟著設定改變，並顯示暱稱/職業）。
- [x] 整理哪些狀態要放 store、哪些保留元件內（使用者資料進 store，其餘互動紀錄仍在頁面內）。
- [x] 更新 `docs/DEVLOG.md`。

### 驗收

- [x] 不同元件能共用同一份資料（設定面板與首頁共用 user store）。
- [x] 修改設定後首頁內容同步更新（下班時間、暱稱/職業）。
- [x] 能說清楚 Pinia 和 local state 的差異（全域共享 vs 元件內狀態）。
